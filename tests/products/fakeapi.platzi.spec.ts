...
test.describe("Products tests with Clean Up(?)", {tag: ["@products", "@cleanUp"]}, () => {
...
test.describe("Create products", { tag: ["@products", "@cleanUp"] }, () => {
  ...
  test("Create product", async ({ request }) => {
    ...
    await test.step("Check product exists", async () => {
      const responseGetById = await request.get(
...
test.describe("Delete products", { tag: ["@products"] }, () => {
  let productIdAndSlug;
  let uniqueTitle: string;
  ...
  test("Delete product", async ({ request }) => {
    await test.step("Delete product by id", async () => {
    ...
    await test.step("Check product by Id is NOT exists", async () => {
      const responseGetById = await request.get(
        `/api/v1/products/${productIdAndSlug!.productId}`,
      );
...
test.describe("Get products related by id", { tag: ["@products"] }, () => {
  test("Get products related by id", async ({ request }) => {
    const objects: any[] = [];
    const productIds: number[] = [];

    await test.step("Create 3 products in same category #3", async () => {
      for (let i = 0; i < 3; i++) {
